...
class WU_Gateway_Stripe extends WU_Gateway {
  ...
  public function get_current_payment_method($customer_id) {
    ...
    try {
      ...
      $sub = WU_Stripe\Subscription::all([
        'customer' => $customer_id,
      ]);
      
      $payment_method->invoice_settings->default_payment_method = $sub->data[0]->default_payment_method;
      $payment_method->save();
      
      if (isset($payment_method->default_source->object) && $payment_method->default_source->object == 'card') {
  ...
  public function create_stripe_session() {
    ...
    $customer = false;
		...
		if (isset($this->integration_key)) {

			// Get the customer
			$customer = $this->get_stripe_customer($this->integration_key);
  ...
  public function handle_notifications() {
    ...
    switch ($event->type) {
      ...
      case 'checkout.session.completed':
        ...
        $cards = WU_Stripe\Customer::allSources(
          $customer_id,
        ...
        $update = WU_Stripe\Customer::update(
          $customer_id,
          ['default_source' => $cards->data->id]
        );
